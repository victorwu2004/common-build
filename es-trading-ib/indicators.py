...
class TechnicalIndicators:
    ...
    def calculate_vwap(
        ...
        for i in range(len(closes)):
            ...
            cumulative_tp_volume += typical_price * vol
    ...
    def calculate_all(
        ...
        # Extract OHLCV data (IB format uses different keys)
        opens = [float(bar.get("Open", bar.get("open", 0))) for bar in bars]
        highs = [float(bar.get("High", bar.get("high", 0))) for bar in bars]
        lows = [float(bar.get("Low", bar.get("low", 0))) for bar in bars]
        closes = [float(bar.get("Close", bar.get("close", 0))) for bar in bars]
        volumes = [float(bar.get("Volume", bar.get("volume", 0))) for bar in bars]
