# Short Agent - Trades Short Positions Only
# Looks for bearish signals to enter short positions
...
class ShortAgent:
    ...
    Entry conditions (bearish signals):
    - EMA fast < EMA slow (downtrend)
    - RSI > 30 (not oversold) and RSI < 70 (declining from overbought)
    - Price below VWAP
    ...
    def _check_entry_signals(
        ...
        if indicators.ema_fast < indicators.ema_slow:
            ...
            return TradeSignal(
                ...
                side=OrderSide.SELL_SHORT,
                quantity=0,
                reason="EMA bullish - no short entry"
            )
        
        # 2. RSI Conditions
        if indicators.rsi > self.config.RSI_OVERSOLD:
            if indicators.rsi < 60:  # Declining but not oversold
                score += 0.15
                reasons.append(f"RSI favorable ({indicators.rsi:.1f})")
            if 55 < indicators.rsi < 70:  # Falling from overbought
                score += 0.15
        ...
        if indicators.adx > 20:
            if indicators.minus_di > indicators.plus_di:
                score += 0.15
                reasons.append(f"ADX trending bearish ({indicators.adx:.1f})")
        
        # 6. Bollinger Bands
        if current_price > indicators.bollinger_middle:
            score += 0.05
            reasons.append("Above BB middle (value short)")
        ...
        if score >= 0.5:
            ...
            stop_loss = bid_price + (atr * 2)  # Stop above entry for shorts
            take_profit = bid_price - (atr * 3)  # Target below entry for shorts
    ...
    def _check_exit_signals(
        ...
        # 5. MACD Bullish Crossover with profit
        if indicators.macd > indicators.macd_signal and pnl_ticks > 8:
            should_exit = True
            reasons.append("MACD bullish + profit")
        
        # 6. Price above VWAP with profit
        if current_price > indicators.vwap and pnl_ticks > 4:
            reasons.append("Above VWAP (warning)")
